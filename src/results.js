...
import { graphElement, replayElement } from "./dom.js";
...
const updateGraph = () => {
  let y = 0;
  const rects = Object.entries(RUN.results)
    .map(([state, count]) => {
  ...
  if (RUN.tick < TOTAL_TICKS) {
    Object.entries(domElements).forEach(([state, domElement]) => {
      if (domElement) {
        ...
        if (RUN.results[STATES.infected] > RUN.results["max-infected"]) {
          RUN.results["max-infected"]++;
        }
